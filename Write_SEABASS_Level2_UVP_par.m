...
for nline = 1:20
  odv_dr{nline} = fgetl(fileID);
end
fclose(fileID);
column_hdrline = find(contains(odv_dr,'Cruise'));
% resize header
odv_dr = odv_dr(1:column_hdrline);
cols = strsplit(odv_dr{column_hdrline},table_options.Delimiter); % split odv_dr by odv delimiter
for ic = 1:numel(cols)
...
% commonly data in the 50.8-64? size bin and we want to maintain all
...
% Since the smallest size bin we are considering is 50.8-64? and a pixel
% is ~0.01?, ignore the contribution to uncertainty based on size.
...
u_VSD  = rel_unc .* VSD;  % [%]*[mm^3/L]    = [mm^3/L] convert to [uL/m^3] later
u_DNSD = rel_unc .* DNSD; % [%]*[#/m^3/um]  = [#/m^3/um]
u_DVSD = rel_unc .* DVSD; % [%]*[uL/m^3/um] = [uL/m^3/um]
...
u_DNSD_text  = {'!  uncertainty in the differential number size distribution [#/m^3/um]';...
                '!  PSD_DNSD_{#}umsize_unc = relative_unc*PSD_DNSD_{#}umsize'};
u_VSD_text   = {'!  uncertainty in the volume size distribution [uL/m^3]';...
                '!  PSD_VSD_{#}umsize_unc = relative_unc*PSD_VSD_{#}umsize'};
u_DVSD_text  = {'!  uncertainty in the differential volume size distribution [uL/m^3/um]';...
...
% %   volumetric concentration (uL/l) using the median ESD (mm) of each
% %   logarithmically-spaced size bin and assuming spherical shape using the
% %   following equation"
% %   V = N*4/3*pi*(d/2)^3
% %   For each UVP size class, Vis the particle volume concentration (uL/ l),
...
%   u_DVSD(:,ns) = u_VSD(:,ns) ./ wid_mm(ns); % [mm^3/L/mm] == [uL/m^3/um]
% end
...
hdr_SEABASS={'/begin_header';
  ...
  ['/data_status='        hdr.data_status.PAR];
  ['/start_date='         datemin];
  ['/end_date='           datemax];
  ...
  '!'};
if include_uncertainty
  hdr.comments.PAR = [hdr.comments.PAR; ...
    '!';...
...
hdr_SEABASS = [hdr_SEABASS; hdr.comments.PAR;...
  '!';
  ['/fields=' colstr];
  ['/units=' colunits];
  '/end_header'];
...
% VSD from [mm^3/L] to [uL/m^3] --> VSD * 1000.0; (1uL=1mm^3, 1000L=1m^3)
